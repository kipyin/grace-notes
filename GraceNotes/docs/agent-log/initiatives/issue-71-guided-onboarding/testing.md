...
- **Fixes applied**: skip post–Seed full-screen cover under UI test (`ProcessInfo.graceNotesIsRunningUITests`); stable UI-test SwiftData store across `terminate()`/`launch()` (session key marker in `PersistenceController`); `configureUITestLaunch` before every app `launch()`; English locale launch args; gratitude chip ids `JournalGratitudeChip.*` for persistence assertions; `ReviewScreen` keeps list chrome when `FIVECUBED_UI_TESTING` and entries are empty; keyboard test re-taps gratitude after guided focus moves to Need; persistence test asserts `JournalGratitudeChip.0` on Today after relaunch (avoids flaky Review navigation).
