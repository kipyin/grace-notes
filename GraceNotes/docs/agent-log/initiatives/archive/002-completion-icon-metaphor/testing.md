...
- `JournalCompletionLevelTests.test_completionStatusSystemImage_matchesCompletionIconDesign` — locks SF Symbol names for each `JournalCompletionLevel` and both `isEmphasized` values per `GraceNotes/docs/agent-log/initiatives/archive/002-completion-icon-metaphor/design.md`.
