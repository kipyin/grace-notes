...
**Merge gates:** GitHub checks must be green. Unresolved **Copilot** review threads block the merge unless you configure `copilot_login` to match the bot’s `author.login` in the API (use `grace sentry review-thread-authors <PR#>` on a real PR to see those logins). **Cursor** (optional): the GitHub App posts as **`cursor[bot]`** (also listed with fallbacks `cursor` and `cursoragent`); use `grace sentry issue-comment-authors <PR#>` on a PR where Cursor commented to confirm `user.login`, then set `cursor_reviewer_logins` if needed. After opening a PR, sentry posts `/review` once (when `cursor_post_review_trigger` is true and logins are non-empty) to trigger Cursor’s review. The merge poll waits only if Cursor posted a “started” phrase (default includes “Taking a look”) in **issue comments** and has not yet followed up with another issue comment or substantive text in the same comment—**or** Cursor has not submitted a **PR review** yet (GitHub review states other than `PENDING`). That covers quick reviews where the starter comment is deleted or the review is only on the PR review (not duplicated in issue comments). If Cursor never shows that start signal in issue comments and has not submitted a PR review, the Cursor gate does not block. Set `cursor_reviewer_logins = []` to disable Cursor entirely. Allowlisted users can still post the approval phrase (default `/sentry-approve`) to unblock if a reviewer is stuck.
