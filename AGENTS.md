...
CI uses **GitHub Actions** with **`make lint`** + **`make ci-build`** on PRs to **`main`**, **`make ci-merge-queue`** on **`merge_group`** / **`full-ci`** / rare **`main`** pushes. Runners select **Xcode 26.3** and use **iPhone 17 Pro @ iOS 26.2** plus **iPhone SE (3rd generation) @ iOS 18.5** (`CI_SIMULATOR_PRO` / `CI_SIMULATOR_XR`) without downloading simulator platforms in workflow steps. See **CI (GitHub Actions)** in [`README.md`](README.md).
