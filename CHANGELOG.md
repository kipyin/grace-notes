...
Insight quality, guided first-week onboarding, and a **0.5.0** release at **build 7** (git tag **v0.5.0+7**). The 2026-03-21 integration established Review and tutorial scope; this entry adds upgrade orientation keyed to marketing **0.5.0** and bundle **7** (`lastLaunchedMarketingVersion`, `lastLaunchedBundleVersion`), plus packaging, localization, and fixes previously summarized under interim **0.5.1** / build **3** labeling.

### Added
- Journal: **one-time upgrade orientation** on the first launch of **0.5.0** build **7** after an older marketing version or an older **0.5.0** build. Users still below **Seed** on Today keep the full guided chip path; users **at or above Seed** see the post-Seed settings-oriented journey **without** the Seed congratulations page. Launch tracks marketing and bundle versions; migration into `completedGuidedJournal` is deferred until Today’s completion level is known for that cohort.
...
- App **marketing version** `0.5.0`; **bundle version** (`CURRENT_PROJECT_VERSION`) `7` for Grace Notes app configurations (Debug, Release, Demo). Git release tag **`v0.5.0+7`**. `OrientationReleaseGate` + `lastLaunchedBundleVersion` gate upgrade orientation across **0.5.0** build bumps.
